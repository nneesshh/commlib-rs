...
//! use libc::{c_int, c_void, size_t, ssize_t, sockaddr};
        ...
        unsafe {
            match self.sa6.sin6_family as i32 {
                ...
                AF_INET6 => {
                    ...
                    #[cfg(target_os = "windows")]
                    let (ip, scope_id) =
                        (*self.sa6.sin6_addr.u.Byte(), *self.sa6.u.sin6_scope_id());
...
mod tests {
    ...
    fn convert_from_and_to_raw() {
        ...
        where
            F: Fn(&mut [u8], *mut sockaddr, *mut socklen_t),
        {
            let mut buf = Vec::new();
            buf.resize(len, 0xff);
        ...
        with_buf(128, |_, sa, sa_len| unsafe {
            assert_eq!(
                OsSocketAddr::new().copy_to_raw(sa, sa_len),
                Err(BadFamilyError(0))
            );
            assert_eq!(*sa_len, 0);
        });
        // null pointers
        unsafe {
            osa4.copy_to_raw(std::ptr::null_mut(), std::ptr::null_mut())
                .unwrap();
        }
