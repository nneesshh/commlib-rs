...
CBlowfish& CBlowfish::operator=(CBlowfish&& orig)
{
    if (this != &orig)
    {
        // Release any existing state owned by the current instance
        if (state != nullptr)
        {
            delete state;
        }
...
uint64_t CBlowfish::encrypt64(const uint64_t data) noexcept
{
    ...
    encrypt(&data_l, &data_r);

    uint64_t encrypted = (static_cast<uint64_t> (data_l) << 32) |
                         (static_cast<uint64_t> (data_r));
...
uint64_t CBlowfish::decrypt64(const uint64_t data) noexcept
{
    ...
    decrypt(&data_l, &data_r);

    uint64_t decrypted = (static_cast<uint64_t> (data_l) << 32) |
                         (static_cast<uint64_t> (data_r));
...
void CBlowfish::encrypt(uint32_t* const data_l_ref, uint32_t* const data_r_ref) noexcept
{
    ...
    for (size_t p_box_index = 0; p_box_index < ROUNDS; p_box_index += UNROLLED_STEP)
    {
        data_l ^= state->p_box[p_box_index];
        data_r ^= blowfish_f(data_l);
        data_r ^= state->p_box[p_box_index + 1];
        data_l ^= blowfish_f(data_r);
    }
...
void CBlowfish::decrypt(uint32_t* const data_l_ref, uint32_t* const data_r_ref) noexcept
{
    ...
    for (size_t p_box_index = ROUNDS;
         p_box_index >= UNROLLED_STEP;
         p_box_index -= UNROLLED_STEP)
    {
...
void CBlowfish::set_key(const char* const key, const size_t key_length) noexcept
{
    ...
    for (size_t p_index = 0; p_index < P_BOXES; p_index += UNROLLED_STEP)
    {
        encrypt(&data_l, &data_r);
    ...
    for (size_t s_box_index = 0; s_box_index < S_BOXES; ++s_box_index)
    {
        for (size_t s_entry_index = 0;
             s_entry_index < S_BOX_ENTRIES;
             s_entry_index += UNROLLED_STEP)
        {
            encrypt(&data_l, &data_r);
...
const CBlowfish::bf_state CBlowfish::INIT_STATE =
{
    ...
    // S boxes 1 - 4 (s_box[0][] - s_box[3][])
    {
        // S box 1 entries (s_box[0][0] - s_box[0][255])
        {
        ...
        // S box 2 entries (s_box[1][0] - s_box[1][255])
        {
        ...
        // S box 3 entries (s_box[2][0] - s_box[2][255])
        {
        ...
        // S box 4 entries (s_box[3][0] - s_box[3][255])
        {
