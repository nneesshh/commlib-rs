...
pub struct DiscoveryServer {
    ...
    participants: hashbrown::HashMap<String, ParticipantInfo>,
}
...
impl DiscoveryServer {
    pub fn new() -> io::Result<DiscoveryServer> {
        ...
        Ok(DiscoveryServer {
            ...
            participants: hashbrown::HashMap::new(),
        })
    }
    ...
    fn register(&mut self, name: &str, addr: SocketAddr, endpoint: Endpoint) {
        if !self.participants.contains_key(name) {
            ...
            for participant in &mut self.participants {
                self.handler
                    .network()
                    .send(participant.1.endpoint, &output_data);
            }
    ...
    fn unregister(&mut self, name: &str) {
        if let Some(info) = self.participants.remove(name) {
            ...
            println!(
                "Can not unregister an non-existent participant with name '{}'",
                name
            );
        }
    }
