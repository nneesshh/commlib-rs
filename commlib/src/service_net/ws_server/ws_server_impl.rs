...
use crate::{ConnId, ListenerId, ServerStatus, ServiceNetRs, ServiceRs, TcpConn};

use super::ws_server_manager::ws_server_make_new_conn;
...
impl WsServer {
    ...
    pub fn on_ll_input(
        ...
        packet_builder: &mut WsPacketBuilder,
    ) {
        // 运行于 srv_net 线程
        assert!(self.srv_net.is_in_service_thread());
        ...
        F: Fn(ListenerId, SocketAddr) + Send + Sync + 'static,
    {
        ...
        let listen_fn = move |r| {
            match r {
                Ok((listener_id, sock_addr)) => {
                    // 状态：Running
                    ws_server.set_status(ServerStatus::Running);
        ...
        let accept_fn = move |_listener_id, hd, sock_addr| {
            // make new connection
            ws_server_make_new_conn(&ws_server2, hd, sock_addr);
        };
        ...
        listener.listen_with_ws(self.addr.as_str());
    }
